	...
	<div>
		<v-card class="my-6">
			<v-card-title class="text-h5" @click.self="visible = !visible" v-ripple style="cursor: pointer">
				<h5 @click.self="visible = !visible" class="text-h5">Pit Scouting Assignments</h5>
				<v-spacer></v-spacer>
				...
				<template v-slot:default>
					...
					<tbody>
						<tr
							...
							"
						>
							<td v-ripple style="cursor: pointer" @click="$router.push(`/events/${event.key.toUpperCase()}/teams/${team.key.substring(3)}`)" class="font-weight-black">
								{{ team.key.substring(3) }}
							</td>

							<td>
								{{ scoutingData[team.key].unassigned ? "-" : scoutingData[team.key].assignedName }}
							</td>

							<td
								:class="{
									'font-weight-black grey--text text--darken-1 font-italic': scoutingData[team.key].unassigned,
									'red--text': !scoutingData[team.key].unassigned && !scoutingData[team.key].submitted,
									'green--text': scoutingData[team.key].submitted,
								}"
							>
								{{ scoutingData[team.key].unassigned ? "Unassigned" : scoutingData[team.key].submitted ? "Submitted" : "Not Submitted" }}
							</td>
						</tr>
					</tbody>
				</template>
		...
		<v-dialog v-model="change.open" v-if="change.open" max-width="400px">
			<v-card>
				<v-card-title
					><h4 class="text-h4 font-weight-black mt-1">TEAM {{ change.team.substring(3) }}</h4></v-card-title
				>

				<v-card-text>
					<v-select label="Assigned to..." v-model="change.to" :items="reassignOptions" hide-details></v-select>

					<v-btn class="mt-8" color="secondary" block @click="openForm()">Open Form<v-icon right>mdi-arrow-top-right</v-icon></v-btn>
				</v-card-text>
		...
		<v-dialog scrollable v-model="autoOpen" v-if="auto" max-width="500px">
			<v-card>
				...
				<v-card-text>
					<v-checkbox v-for="(value, key) in auto" :key="key" v-model="auto[key]" :label="autoEnum[key]" hide-details dense></v-checkbox>

					<p class="red--text text-center mt-3 mb-0 pb-0">Warning! This will overwrite any existing pit scouting assignments.</p>
				</v-card-text>
	...
	export default {
		...
		methods: {
			refresh() {
				...
				this.$http.get(`/api/event/${this.event.key}/teams/short`).then((response) => {
			...
			deleteScoutingData() {
				this.$http.delete(`/api/scouting/pit/${this.event.key}?team=${this.change.team}`).then(() => {
					this.$emit("refresh");
				});
			},
			openForm() {
				this.$router.push(`/events/${this.event.key.toUpperCase()}/scouting/pit?team=${this.change.team.substring(3)}`);
			},
			autoAssign() {
				this.$http.post(`/api/scouting/genpit/${this.event.key}`, this.auto).then(() => {
					this.$emit("refresh");
					this.autoOpen = false;
				});
			},
			changeAssignment() {
				this.$http.post(`/api/scouting/pitscouting/${this.event.key}`, this.change).then(() => {
					this.$emit("refresh");
