	...
	<v-card class="mb-4" style="white-space: nowrap">
		...
		<v-simple-table dense v-if="visible && ranking && ranking.length != 0">
			<template v-slot:default>
				...
				<tbody>
					<tr v-ripple @click="$router.push(`/events/${event.key.toUpperCase()}/teams/${team.team_key.substring(3)}`)" style="cursor: pointer" v-for="team in ranking" :key="team.team_key">
		...
		<v-card-text v-else-if="visible && (!ranking || ranking.length == 0)">
			<p class="text-center mt-2 grey--text font-italic">Ranking data unavailable</p>
		</v-card-text>
	</v-card>
</template>
	...
	export default {
		...
		methods: {
			refresh() {
				...
				this.$http.get(`/api/event/${this.$route.params.code.toLowerCase()}/ranking`).then((response) => {
