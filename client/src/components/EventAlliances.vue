<template>
  <v-card class="mb-4" style="white-space: nowrap">
    ...
    <v-simple-table
      dense
      v-if="visible && event.alliances && event.alliances.length != 0"
    >
      <template v-slot:default>
        ...
        <tbody v-if="!event.division_keys">
          <tr
            ...
            :class="{
              'font-weight-black':
                alliance.status.level == 'f' && alliance.status.status == 'won',
            }"
            :style="
              alliance.status.level == 'f'
          ...
          >
            ...
            <td
              @click="
                $router.push(
                  `/events/${event.key.toUpperCase()}/teams/${alliance.picks[0].substring(
                    3
                  )}`
                )
              "
            ...
            <td
              @click="
                $router.push(
                  `/events/${event.key.toUpperCase()}/teams/${alliance.picks[1].substring(
                    3
                  )}`
                )
              "
            ...
            <td
              @click="
                $router.push(
                  `/events/${event.key.toUpperCase()}/teams/${alliance.picks[2].substring(
                    3
                  )}`
                )
              "
        ...
        <tbody v-else>
          <tr
            ...
            @click="
              $router.push(
                `/events/${event.division_keys[index].toUpperCase()}`
              )
            "
          >
            <td class="font-weight-black">{{ alliance.name }}</td>
    ...
    <v-card-text
      v-else-if="visible && (!event.alliances || event.alliance.length == 0)"
    >
      <p class="text-center mt-2 grey--text font-italic">
        Alliance data unavailable
      </p>
    </v-card-text>
  </v-card>
</template>
