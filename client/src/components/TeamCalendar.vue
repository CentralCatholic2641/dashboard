		...
		<v-row class="fill-height ma-0 pa-0">
			<v-col class="ma-0 pa-0">
				<v-toolbar flat class="transparent">
					...
					<v-spacer></v-spacer>
					<div class="d-flex" style="align-items: center" v-if="$vuetify.breakpoint.mdAndUp">
						...
						<v-btn :disabled="focus == ''" outlined class="ml-4" @click="setToday"> Today </v-btn>
					</div>
				</v-toolbar>

				<v-sheet height="575">
					<v-calendar ref="calendar" v-model="focus" color="primary" :events="events" type="month" @click:event="showEvent"></v-calendar>
					<v-menu v-model="selectedOpen" :close-on-content-click="false" :activator="selectedElement" offset-x>
						<v-card width="350px" flat>
							...
							<v-card-text>
								...
								<p class="mb-0 mt-2 pa-0 font-italic" v-if="selectedEvent.color == '#DC2626'">Bonus meeting</p>
							</v-card-text>

							<v-card-actions v-if="$root.user.role > 2">
								<v-btn color="primary" link :to="`/meetings/${selectedEvent._id}`">Open<v-icon right>mdi-arrow-right</v-icon></v-btn>
							</v-card-actions>
						</v-card>
					</v-menu>
				</v-sheet>
				...
				this.$http.get("/api/calendar").then((response) => {
					...
					for (let i of this.meetings) {
						this.events.push({
							...
							timed: true,
							details: dayjs(`${i.date}`).format("ddd, MMM D, YYYY") + " at " + i.start + " to " + i.end,
							_id: i._id,
						});
					}
...
<style>
	...
	.theme--dark.v-calendar-weekly:not(.v-outside) .v-calendar-weekly__day:not(.v-outside) {
