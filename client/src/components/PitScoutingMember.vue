  ...
  <v-card class="my-6">
      ...
      <template v-slot:default>
        ...
        <tbody>
          <tr
            ...
            @click="
              $router.push(
                `/events/${event.key.toUpperCase()}/scouting/pit?team=${team.key.substring(
                  3
                )}`
              )
            "
    ...
    <v-card-text v-else-if="visible">
      <p class="grey--text font-italic text-center my-6">
        You are not assigned to scout any team's pit
      </p>
    </v-card-text>
  </v-card>
</template>
    ...
    refresh() {
      ...
      for (let key in this.pitscouting) {
        if (
          ...
          this.pitscouting[key].assigned == this.$root.user.username
        ) {
          data.push({
            key,
            ...this.pitscouting[key],
      ...
      data.sort(
        (a, b) => parseInt(a.key.substring(3)) - parseInt(b.key.substring(3))
      );
      this.myTeams = data;
    },
  },
