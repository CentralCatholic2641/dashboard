			...
			<v-simple-table dense v-if="visible" style="white-space: nowrap">
				<template v-slot:default>
					...
					<tbody v-if="$root.user.role > 2">
						<tr v-for="member in leaderboard" :key="member._id" @click="$router.push(`/members/${member._id}`)" style="cursor: pointer" v-ripple>
							<td class="font-weight-black">{{ member.rank }}</td>
							<td class="font-weight-black">{{ member.name }}</td>
							<td>
								{{ member.meetings }}<span class="grey--text" v-if="member.bonus != 0">&nbsp;+{{ member.bonus }}</span>
							</td>
							<td>{{ member.hours }}</td>
							<td :class="getColor((member.rawHours / possibleHours) * 100)" v-if="possibleHours != 0">{{ ((member.rawHours / possibleHours) * 100).toFixed(1) }}%</td>
							<td v-else>-</td>
						</tr>
					</tbody>
					...
					<tbody v-else>
						<tr v-for="member in leaderboard" :key="member._id">
							...
							<td>{{ member.hours }}</td>
							<td :class="getColor((member.rawHours / possibleHours) * 100)">{{ ((member.rawHours / possibleHours) * 100).toFixed(1) }}%</td>
						</tr>
					</tbody>
				</template>
			</v-simple-table>
