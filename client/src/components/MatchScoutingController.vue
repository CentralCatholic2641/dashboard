          ...
          <tbody>
            <tr v-for="(match, index) in event.schedule" :key="match.key">
              <td
                ...
                @click="
                  $router.push(
                    `/events/${event.key.toUpperCase()}/schedule/${match.key
                      .substring(match.key.lastIndexOf('_') + 1)
                      .toUpperCase()}`
                  )
                "
              ...
              >
                <span
                  v-if="match.comp_level != 'f' && match.comp_level != 'qm'"
                >
      ...
      <v-card>
        <v-card-title
          ><h4 class="text-h4 font-weight-black mt-1">
            {{
              change.match
                .substring(change.match.lastIndexOf("_") + 1)
                .toUpperCase()
            }}
        ...
        <v-card-text>
          ...
          <div class="d-flex">
            ...
            > -->
            <v-btn class="mt-8" color="secondary" @click="openForm()"
      ...
      <v-card>
        ...
        <v-card-text>
          <v-checkbox
            ...
            :label="autoEnum[key]"
            hide-details
            :disabled="autoCount >= 6 && !auto[key]"
            @change="!auto[key] ? autoCount-- : autoCount++"
            dense
          ></v-checkbox>

          <p class="red--text text-center mt-3 mb-0 pb-0">
            Warning! This will overwrite any existing schedule.
          </p>
        </v-card-text>
...
export default {
  ...
  methods: {
    ...
    getScoutingData(match, station) {
      ...
      if (scout) {
        if (scout[station] && scout[station].assigned.length != 0)
          return scout[station];
        else
          return {
            ...scout,
            unassigned: true,
    ...
    deleteScoutingData() {
      ...
      const stationIndex =
        alliance == "red"
          ? parseInt(this.change.station.substring(3)) - 1
          : parseInt(this.change.station.substring(4)) - 1;

      const team = this.event.schedule
        .find((i) => i.key == this.change.match)
        .alliances[alliance].team_keys[stationIndex].substring(3);
      this.$http
        .delete(
          `/api/scouting/match/${this.change.match}?team=${team}&station=${this.change.station}`
        )
        .then(() => {
          this.$emit("refresh");
          this.change = {
    ...
    openForm() {
      ...
      this.$router.push(
        `/events/${this.event.key.toUpperCase()}/scouting/match?match=${this.change.match
          .substring(this.change.match.lastIndexOf("_") + 1)
          .toUpperCase()}&team=${team}`
      );
    },
    autoAssign() {
      this.$http
        .post(`/api/scouting/genmatch/${this.event.key}`, this.auto)
        .then((response) => {
    ...
    changeAssignment() {
      this.$http
        .post(`/api/scouting/schedule/${this.event.key}`, this.change)
        .then(() => {
