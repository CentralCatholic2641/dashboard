        ...
        <template v-slot:default>
          ...
          <tbody>
            <tr
              ...
              :key="user._id"
              :class="{
                'grey--text text--darken-1 font-italic':
                  assignments[user.username] == 0,
              }"
            >
              <td
              ...
              <td :class="getColor(user.username)">
                {{
                  (
                    (submitted[user.username] / assignments[user.username]) *
                    100
                  ).toFixed(1)
                }}%
              </td>
...
export default {
  ...
  methods: {
    getColor(username) {
      const grade =
        (this.submitted[username] / this.assignments[username]) * 100;
  ...
  computed: {
    assignments() {
        ...
        if (this.schedule[j]) {
          for (let k in this.schedule[j]) {
            if (this.schedule[j][k] && this.schedule[j][k].assigned) {
              if (typeof data[this.schedule[j][k].assigned] != "number")
                data[this.schedule[j][k].assigned] = 0;
      ...
      for (let j in this.pitscouting) {
        if (this.pitscouting[j] && this.pitscouting[j].assigned) {
          if (typeof data[this.pitscouting[j].assigned] != "number")
            data[this.pitscouting[j].assigned] = 0;
    ...
    scouts() {
      return this.users.filter(
        (i) => i.includeInScouting && this.assignments[i.username] != 0
      );
    },
  },
};
