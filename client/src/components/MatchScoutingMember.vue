<template>
  <v-card class="my-6">
          ...
          <tr
            ...
            @click="
              $router.push(
                `/events/${
                  event.year
                }${event.event_code.toUpperCase()}/scouting/match?match=${match.key
                  .substring(match.key.lastIndexOf('_') + 1)
                  .toUpperCase()}&team=${match.team}`
              )
            "
          >
    ...
    <v-card-text v-else-if="visible">
      <p class="grey--text font-italic text-center my-6">
        You are not assigned to scout any matches
      </p>
    </v-card-text>
  </v-card>
</template>
...
export default {
  ...
  methods: {
    refresh() {
      ...
      for (let key in this.schedule) {
        for (let station in this.schedule[key]) {
          if (
            ...
            this.schedule[key][station].assigned == this.$root.user.username
          ) {
            const eventData = this.event.schedule.find((i) => i.key == key);
            const alliance = station.includes("Red") ? "red" : "blue";
            const stationIndex =
              alliance == "red"
                ? parseInt(station.substring(3)) - 1
                : parseInt(station.substring(4)) - 1;

            data.push({
              ...eventData,
              ...this.schedule[key][station],
              time: dayjs(eventData.actual_time * 1000).format(
                "M/D/YY [at] HH:mm"
              ),
              station:
                (station.includes("Red") ? "Red" : "Blue") +
                " " +
                (stationIndex + 1),
              team: eventData.alliances[alliance].team_keys[
                stationIndex
              ].substring(3),
            });
          }
      ...
      data.sort(
        (a, b) => dayjs(a.actual_time * 1000) - dayjs(b.actual_time * 1000)
      );
      this.myMatches = data;
    },
  },
