  ...
  <div v-if="meetings && leaderboard && myStats">
    <v-card>
      <v-card-title
        ...
        v-if="!$route.path.includes('/members')"
      >
      ...
      <v-card-title v-else
        ><h5 class="text-h5">Attendance Stats</h5></v-card-title
      >
      ...
      <div v-if="visible">
          ...
          <div class="d-flex mb-4">
            <div class="mr-5">
              <span>
                ...
                <p class="d-inline grey--text" v-if="myStats.bonus != 0">
                  +{{ myStats.bonus }}
                </p>
              </span>
              <p class="pa-0 mb-0 mt-1">
                Meeting{{ myStats.meetings == 1 ? "" : "s" }}
              </p>
            </div>

            <div class="ml-5">
              <h3
                class="text-h3 font-weight-light mb-1"
                :class="getColor()"
                v-if="possibleHours != 0"
              >
                {{ ((myStats.rawHours / possibleHours) * 100).toFixed(0) }}%
              </h3>
        ...
        <v-simple-table
          ...
          v-if="
            myStats.meetings != 0 &&
            ($route.path.includes('/attendance')
              ? $root.user.role > 2
                ? false
                : true
              : true)
          ...
          <template v-slot:default>
            <thead>
              <tr>
                ...
                <th v-if="!$route.path.includes('/members')" class="text-left">
                  Attended
                </th>
              </tr>
            </thead>
            <tbody v-if="$root.user.role > 2">
              <tr
                v-for="meeting in meetings.filter((i) =>
                  $route.path.includes('/members')
                    ? !hasNotHappened(i._id) && i.attendance[user]
                    : true
                )"
                :key="meeting._id"
                :class="{
        ...
        <v-card-text v-if="visible && myStats.meetings == 0">
          <p
            class="text-center grey--text font-italic pa-0 mb-0 mt-n6"
            v-if="!$route.path.includes('members')"
          >
            You have not attended any meetings
          </p>
          <p class="text-center pa-0 mb-0 mt-n6 grey--text font-italic" v-else>
