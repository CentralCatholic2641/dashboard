<template>
	<v-card class="mb-4" style="white-space: nowrap">
		...
		<v-simple-table dense v-if="visible && playoffs.length != 0">
			<template v-slot:default>
				...
				<tbody>
					...
					<tr @click="$router.push(`/events/${event.year}${event.event_code.toUpperCase()}/schedule/${match.key.toUpperCase().substring(match.key.indexOf('_') + 1)}`)" v-ripple style="cursor: pointer" v-for="match in playoffs" :key="match.key">
						<!-- <td style="width: 20px">
		...
		<v-card-text v-else-if="visible && playoffs.length == 0">
			<p class="text-center mt-2 grey--text font-italic">Playoff results unavailable</p>
		</v-card-text>
	</v-card>
</template>
...
<script>
	export default {
		...
		computed: {
			playoffs() {
				let data = JSON.parse(JSON.stringify(this.event.schedule.filter((i) => i.comp_level != "qm")));

				const qf = data.filter((i) => i.comp_level == "qf").sort((a, b) => a.set_number - b.set_number);

				const sf = data.filter((i) => i.comp_level == "sf").sort((a, b) => a.set_number - b.set_number);

				const f = data.filter((i) => i.comp_level == "f").sort((a, b) => a.set_number - b.set_number);
