	...
	<div>
		<v-toolbar dense color="grey darken-4" style="z-index: 1">
			<v-toolbar-title class="d-flex">
				...
				<h6 class="text-h6 font-weight-black">{{ $root.user.role > 2 ? "" : "My" }} Sponsors</h6>
			</v-toolbar-title>
		...
		<div :style="`height: calc(100vh - ${$vuetify.breakpoint.mdAndUp ? '112px' : '104px'}); overflow: auto`">
			<v-container style="max-width: 1185px" class="px-8 pt-0 mt-4">
				<v-card class="mt-6" v-if="$root.user.role > 2 ? sponsors.length != 0 : sponsors.filter((i) => i.sponsor.member == $root.user._id).length != 0">
					<v-simple-table class="transparent" dense style="white-space: nowrap">
						<template v-slot:default>
							...
							<tbody v-if="$root.user.role > 2">
								<tr v-ripple style="cursor: pointer" v-for="sponsor in sponsors" :key="sponsor._id" @click="$router.push(`/sponsors/${sponsor._id}`)">
									...
									<td v-else>-</td>
									<td>
										{{ members.find((i) => i._id == sponsor.sponsor.member) ? members.find((i) => i._id == sponsor.sponsor.member).name : "-" }}
									</td>
									<td>{{ sponsor.name }}</td>
							...
							<tbody v-else>
								<tr v-for="sponsor in sponsors.filter((i) => i.sponsor.member == $root.user._id)" :key="sponsor._id">
									<td class="font-weight-black">
										{{ sponsor.sponsor.org || "-" }}
									</td>
									<td v-if="sponsor.sponsor.level == 'Blue'" class="blue--text text--darken-2 font-weight-black">
				...
				<p class="grey--text font-italic text-center mt-12" v-else>No sponsors</p>

				<p v-if="$root.user.role > 2 ? sponsors.length > 0 : sponsors.filter((i) => i.sponsor.member == $root.user._id).length > 0" class="pb-0 mt-4 mb-1 grey--text">
					{{ $root.user.role > 2 ? sponsors.length : sponsors.filter((i) => i.sponsor.member == $root.user._id).length }}
					sponsors
				</p>

				<v-dialog max-width="700" v-model="new_wizard.open" v-if="new_wizard.open">
					<v-card>
						<NewSponsorWizard :members="members" @close="(new_wizard.open = false), refresh()" />
					</v-card>
				</v-dialog>
			</v-container>
		</div>
