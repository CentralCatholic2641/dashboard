<template>
  <div>
    ...
    >
      <v-container
        style="max-width: 1185px; height: calc(100% - 16px)"
...
export default {
  ...
  methods: {
    refresh() {
      ...
      this.$http
        .get(
          `/api/resource/${encodeURIComponent(this.$route.params.path)}/${
        ...
        .then((response) => {
          if (response.data.type == "Folder")
            this.$router.push(
              `/resources/${encodeURIComponent(response.data.path)}%2F${
          ...
          if (response.data == null) {
            this.$router.push(
              `/resources/${encodeURIComponent(this.$route.params.path)}`
            );
          }
