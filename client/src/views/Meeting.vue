        ...
        <v-row class="mt-4">
          <v-col cols="12" md="8">
                ...
                <v-row @mouseleave="attendance_hover = false" class="pb-4 pt-2">
                  <v-col
                    v-for="n in Math.ceil(
                      users.filter((i) => i.includeInAttendance).length / 14
                    )"
                    :key="n"
                    cols="12"
                    :md="
                  ...
                  >
                    <div
                      v-for="user in users
                        .filter((i) => i.includeInAttendance)
                        .slice(
                          (n - 1) * 14,
                          Math.min(
                            n * 14,
                            users.filter((i) => i.includeInAttendance).length
                          )
                        )"
                      :key="user._id"
                      @mouseover="attendance_hover = user.username"
                    >
                      <v-checkbox
                        :label="user.name"
                        hide-details
                        v-model="meeting.attendance[user._id]"
                        @change="changes = true"
                        dense
                      ></v-checkbox>
                    </div>
            ...
            <v-card class="mb-4" v-else>
              ...
              <v-card-text>
                <span v-if="meeting.attendance[$root.user._id]">
                  You attended this meeting (+{{ duration }} hours)
                </span>
          ...
          <v-col cols="12" md="4" v-if="$root.user.role > 2">
            ...
            <v-list dense class="transparent mt-3 mb-4">
              <v-list-item
                @click="save()"
                :disabled="!changes"
                :class="{ 'blue--text': changes }"
              >
                <v-list-item-icon
                  ><v-icon
                    left
                    :class="{ 'grey--text': !changes, 'blue--text': changes }"
            ...
            <v-img
              v-if="attendance_hover"
              :src="`/api/content/members/${attendance_hover}.jpg`"
            ></v-img>
          </v-col>
        </v-row>
      </v-container>
...
export default {
  ...
  computed: {
    ...
    duration() {
      return dayjs(`${this.meeting.date}T${this.meeting.end}`)
        .diff(`${this.meeting.date}T${this.meeting.start}`, "hour", true)
        .toFixed(2);
    },
  },
  methods: {
    refresh() {
      if (this.$root.user.role > 2) {
        ...
        this.$http
          .get(`/api/meeting/get/${this.$route.params.meeting}`)
          .then((response) => {
