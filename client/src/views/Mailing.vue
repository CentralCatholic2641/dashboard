<template>
	<div>
		<v-toolbar dense color="grey darken-4" style="z-index: 1">
			...
			<v-btn color="primary" :disabled="emails.length == 0" @click="emails = []"><v-icon left>mdi-close</v-icon>Clear</v-btn>
		</v-toolbar>
		...
		<div :style="`height: calc(100vh - ${$vuetify.breakpoint.mdAndUp ? '112px' : '104px'}); overflow: auto`">
			<v-container style="max-width: 1185px" class="px-8 pt-0 mt-4">
				...
				<v-card class="mt-6" v-else>
					<v-card-text class="font-italic text-center">No email addresses added.</v-card-text>
				</v-card>
					...
					<v-col cols="12" md="6">
						<v-card class="mb-6">
							...
							<v-card-text class="text-center">
								<v-btn class="mx-1 mb-2" color="blue darken-4" large @click="mailTo('everyone')">Everyone</v-btn>
								<v-btn class="mx-1 mb-2" color="blue darken-4" large @click="mailTo('members')">Members</v-btn>
								<v-btn class="mx-1 mb-2" color="blue darken-4" large @click="mailTo('mentors')">Mentors</v-btn>
								<v-btn class="mx-1 mb-2" color="blue darken-4" large @click="mailTo('parents')">Parents</v-btn>
								<br />
								<v-btn class="mx-1 mb-2" color="red darken-4" large @click="mailTo('mechanical')">Mechanical</v-btn>
								<v-btn class="mx-1 mb-2" color="red darken-4" large @click="mailTo('electronics')">Electronics</v-btn>
								<v-btn class="mx-1 mb-2" color="red darken-4" large @click="mailTo('code')">Code</v-btn>
							</v-card-text>
						</v-card>
						...
						<v-card>
							...
							<v-simple-table class="transparent" v-if="contacts.length != 0" dense>
								<template v-slot:default>
									<tbody>
										<tr
											v-ripple
											style="cursor: pointer"
											v-for="contact in contacts.filter((i) => i.parents.length != 0)"
											:key="contact._id"
											:class="{
										...
										>
											<td style="width: 75px">
												<v-icon v-if="isIn(contact.parents[0].email)">mdi-check</v-icon>
											</td>
		...
		methods: {
			...
			mailTo(i) {
				...
				switch (i) {
					...
					case "members":
						for (let i of this.contacts) {
							if (i.email && !data.includes(i.email) && i.type != "Mentor") data.push(i.email);
						}
						break;
					case "mentors":
						for (let i of this.contacts) {
					...
					case "mechanical":
						for (let i of this.contacts) {
							if (i.email && !data.includes(i.email) && i.subteam == "Mechanical") data.push(i.email);
						}
						break;
					case "electronics":
						for (let i of this.contacts) {
							if (i.email && !data.includes(i.email) && i.subteam == "Electronics") data.push(i.email);
						}
						break;
					case "code":
						for (let i of this.contacts) {
							if (i.email && !data.includes(i.email) && i.subteam == "Code") data.push(i.email);
						}
						break;
				}
				this.emails = data;
