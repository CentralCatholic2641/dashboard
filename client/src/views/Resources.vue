<template>
  <div>
    <v-toolbar dense color="grey darken-4" style="z-index: 1">
      ...
      <v-btn color="primary" @click="new_wizard = true"
            ...
            <template v-slot:default>
              ...
              <tbody>
                <tr
                  ...
                  @click="
                    $router.push(
                      `/resources/${encodeURIComponent(resource.path)}%2F${
                        resource.filename
                      }`
                    )
                  "
                  @contextmenu="
                    showActionMenu($event), (action_menu_item = resource)
                  "
                >
                  <td>
                ...
                <tr
                  ...
                  @click="
                    resource.type == 'Link'
                      ? open(resource.to)
                      : $router.push(
                          `/resources/${encodeURIComponent(resource.path)}/${
                            resource.filename
                          }`
                        )
                  "
                ...
                >
                  <td>
                    <v-icon v-if="resource.type == 'Document'"
                      >mdi-file-document</v-icon
                    >
                    <v-icon v-else-if="resource.type == 'Image'"
                      >mdi-image</v-icon
                    >
                    <v-icon v-else-if="resource.type == 'Video'"
                      >mdi-video</v-icon
                    >
                    <v-icon v-else-if="resource.type == 'Link'"
    ...
    <v-dialog persistent max-width="450" v-model="new_wizard" v-if="new_wizard">
      <v-card>
        <NewResourceWizard
          @close="(new_wizard = false), refresh()"
    ...
    >
      <v-list dense>
        ...
        <v-list-item @click="newTab(action_menu_item)">
          <v-list-item-icon
            ><v-icon>mdi-arrow-top-right</v-icon></v-list-item-icon
          >
        ...
        <v-list-item
          @click="deleteResource(action_menu_item._id)"
          class="red--text"
          v-if="
            action_menu_item.rights.owner._id == $root.user._id ||
            $root.user.role > 2
...
export default {
  ...
  methods: {
    refresh() {
      ...
      this.$http
        .get(`/api/resource/${encodeURIComponent(this.$route.params.path)}`)
        .then((response) => {
          ...
          const split = ("/" + decodeURIComponent(this.$route.params.path))
            .split("/")
            .slice(1);
          for (let i = 0; i < split.length; i++) {
            this.path.push({
              ...
              to: `/resources/${encodeURIComponent(
                split.reduce((accumulator, currentValue, currentIndex) =>
                  currentIndex > i
                    ? accumulator
    ...
    generateLink(resource) {
      if (resource.type == "Link") return resource.to;
        ...
        return (
          window.location.href + "%2F" + encodeURIComponent(resource.filename)
        );
