	...
	<div v-if="users">
		<v-toolbar dense color="grey darken-4" style="z-index: 1">
			...
			<v-menu offset-y :close-on-content-click="false">
				<template v-slot:activator="{ on, attrs }">
					<v-btn class="mr-2" v-bind="attrs" v-on="on" icon><v-icon>mdi-table</v-icon></v-btn>
				</template>
				<v-list dense>
					<v-list-item v-for="(value, column) of columnMap" :key="column" :input-value="columns[column]" @click="(columns[column] = !columns[column]), updatePageState()">
						<v-list-item-icon><v-icon v-if="columns[column]">mdi-check</v-icon></v-list-item-icon>
						<v-list-item-title>{{ columnMap[column] }}</v-list-item-title>
					</v-list-item>
				</v-list>
			</v-menu>

			<v-btn color="primary" @click="new_wizard.open = true"><v-icon left>mdi-plus</v-icon>New</v-btn>
		</v-toolbar>
		...
		<div :style="`height: calc(100vh - ${$vuetify.breakpoint.mdAndUp ? '112px' : '104px'}); overflow: auto`">
			<v-container style="max-width: 1200px" class="px-8 pt-0 mt-6">
				<v-card v-if="filtered.length > 0">
					<v-simple-table class="transparent" dense style="white-space: nowrap">
						<template v-slot:default>
							<thead>
								<tr>
									<th v-for="(value, column) of columns" v-if="value" :key="column" class="text-left">
										{{ columnMap[column] }}
									</th>
								</tr>
							</thead>
							<tbody>
								<tr v-ripple style="cursor: pointer" v-for="user in filtered" :key="user._id" @click="$router.push(`/members/${user._id}`)">
									<td
										v-for="(value, column) of columns"
										v-if="value"
										:key="column"
										:class="{
											'font-weight-black': column.includes('name') && column != 'username' && column != 'parentname',
										}"
									>
				...
				<p class="font-italic grey--text text-center mt-12" v-else>No members</p>

				<p class="pb-0 mt-4 mb-1 grey--text" v-if="filtered.length > 0">{{ filtered.length }} members</p>

				<v-dialog max-width="700" v-model="new_wizard.open" v-if="new_wizard.open">
					<v-card>
						<new-user-wizard @close="(new_wizard.open = false), refresh()"></new-user-wizard>
					</v-card>
				</v-dialog>
			</v-container>
		</div>
	...
	export default {
		...
		mounted() {
			...
			for (let i of Object.keys(this.columnMap)) {
				if (!Object.keys(this.$root.user.pagestate.members).includes(i)) this.$root.user.pagestate.members[i] = false;
			}
		},
		components: {
			NewUserWizard,
