<template>
	<div v-if="meetings">
		<v-toolbar dense color="grey darken-4" style="z-index: 1">
			...
			<v-btn v-if="$root.user.role > 2" color="primary" @click="create = true"><v-icon left>mdi-plus</v-icon>New</v-btn>
		</v-toolbar>
		...
		<div :style="`height: calc(100vh - ${$vuetify.breakpoint.mdAndUp ? '112px' : '104px'}); overflow: auto`">
			<v-container style="max-width: 1185px" class="px-8 pt-0 mt-4">
				<v-card class="mt-6">
					<v-simple-table class="transparent" v-if="meetings.length != 0" dense style="white-space: nowrap">
				...
				<p v-if="meetings.length > 0" class="pb-0 mt-4 mb-1 grey--text">{{ meetings.length }} meetings</p>
			</v-container>
		</div>
