<template>
  <div>
    <v-toolbar dense color="grey darken-4" style="z-index: 1">
      ...
      <v-btn color="primary" @click="save()" :disabled="cantSave"
      ...
      <v-container style="max-width: 1185px" class="px-8 pt-0 mt-6">
        <v-row>
          <v-col cols="12" md="6">
            ...
            <v-card class="mb-6">
              <v-card-title
                ><h5 class="text-h5">Change Password</h5></v-card-title
              >
...
export default {
  ...
  methods: {
    save() {
      ...
      this.$http
        .put(
          `/api/auth/${this.$root.user._id}?password=${mode}`,
          this.$root.user
        )
        .then((response) => {
          this.$root.user = response.data.find(
            (i) => i._id == this.$root.user._id
          );
          this.changes = false;
          this.reset = {
            old: "",
