<template>
  <div>
    ...
    >
      <v-container class="px-8 pt-0 mt-7" v-if="filtered.length != 0">
        <v-row>
          ...
          >
            <v-card
              :class="getColor(task.subteam)"
              link
              :to="`/tasks/${task._id}`"
              style="height: 100%"
              :style="
                task.assigned.includes($root.user._id)
              ...
              >
                ...
                <v-card-text class="mt-0 pt-0 pb-9">
                  ...
                  <div class="d-flex">
                    <p
                      ...
                      :class="{
                        'orange--text':
                          task.assigned.length < task.peopleNeeded,
                      }"
                    >
                      <v-icon
                        :class="{
                ...
                <div style="position: absolute; bottom: 0px; width: 100%">
                  <div class="d-flex mx-4 mb-3" style="align-items: baseline">
                    ...
                    >
                      {{ getPercentComplete(task).toFixed(0) }}%
                    </p>
                  </div>

                  <v-progress-linear
                    :color="task.status == 'Completed' ? 'grey' : 'white'"
                    rounded
                    :value="getPercentComplete(task)"
                  ></v-progress-linear>
                </div>
              </div>
            </v-card>
        ...
        <p class="pb-0 mt-4 mb-1 ml-1 grey--text">
    ...
    <v-overlay
      ...
      :style="`padding-top: ${$vuetify.breakpoint.mdAndUp ? '25vh' : '0px'}`"
      :opacity="0.65"
    >
      <v-container
        :style="`width: ${$vuetify.breakpoint.mdAndUp ? '1100px' : '100vw'};`"
      ...
      >
        ...
        <v-row>
          <v-col cols="12" md="6">
            <v-card
              v-if="hasRights"
              :class="getColor(current.subteam)"
              class="elevation-12"
            >
              <div
                :class="{
              ...
              >
                  ...
                  <div class="d-flex">
                    ...
                    <p class="d-flex">
                      <v-menu offset-y>
                        ...
                        <v-list dense>
                          <v-list-item
                            v-for="item in subteams"
                            :key="item.value"
                            :value="item.value"
                            @click="
                              (current.subteam = item.value), (changes = true)
                            "
                            :input-value="current.subteam == item.value"
                          >
                            <v-list-item-title>{{
                              item.text
                            }}</v-list-item-title>
                  ...
                  <p class="d-flex">
                    ...
                    >
                      ...
                      <v-date-picker
                        ...
                        v-model="current.due"
                        full-width
                        @click:date="$refs.due_menu.save(current.due)"
                        @change="changes = true"
                        color="primary"
                      ></v-date-picker>
                    </v-menu>
                ...
                <div class="d-flex mx-4 mb-3" style="align-items: baseline">
                  ...
                  <v-spacer></v-spacer>
                  <p
                    v-if="current.status != 'To Do'"
                    class="grey--text ma-0 pa-0"
                  >
                    {{ getPercentComplete(current).toFixed(0) }}%
                  </p>
                </div>
                ...
                <v-progress-linear
                  color="white"
                  rounded
                  :value="getPercentComplete(current)"
                ></v-progress-linear>
              </div>
            </v-card>
                ...
                <v-card-text class="mt-2 pb-9">
                  ...
                  <div class="d-flex">
                    <p
                      ...
                      :class="{
                        'orange--text':
                          current.assigned.length < current.peopleNeeded,
                      }"
                    >
                      <v-icon
                        :class="{
                    ...
                    <p v-if="current.subteam">
                      <v-icon>mdi-account-group</v-icon> {{ current.subteam }}
                    </p>
                  </div>
                  <p
                    v-if="current.due"
            ...
            <div class="mt-6" v-if="hasRights">
              <v-btn text color="red" @click="deleteTask(current._id)"
          ...
          <v-col cols="12" md="6">
            <v-card class="mb-6 elevation-12">
              ...
              <v-simple-table
                ...
                v-if="current.assigned.length != 0"
              >
                <template v-slot:default>
                  ...
                  <tbody>
                    <tr
                      v-for="(member, index) in current.assigned"
                      :key="member._id"
                    >
                      <td
                        @click="setPrimaryAssignee(member)"
                        v-ripple
                      ...
                      <td
                        v-if="hasRights"
                        @click="removeAssignee(member, index)"
              ...
              <v-card-text class="text-center my-0 py-0">
                <p
                  class="grey--text font-italic pb-6"
                  v-if="current.assigned.length == 0"
                >
            ...
            <v-card class="mb-6 elevation-12">
              ...
              <v-simple-table
                ...
                dense
                style="white-space: nowrap"
                v-if="current.checklist.length != 0"
              >
                <template v-slot:default>
                  <tbody v-if="hasRights">
                    ...
                    >
                      <td
                        @click="
                          (item.completed = !item.completed),
                            (changes = true),
                            getPercentComplete(current) == 100
                      ...
                      <td
                        @click="
                          current.checklist.splice(index, 1), (changes = true)
                        "
                        style="cursor: pointer; width: 56px"
                      >
                        <v-icon class="grey--text">mdi-close</v-icon>
              ...
              <v-card-text class="text-center my-0 py-0">
                ...
                <v-text-field
                  ...
                  label="Add..."
                  v-model="add_checklist"
                  hide-details
                  @keypress.enter="addChecklistItem()"
                ></v-text-field>
              </v-card-text>
            </v-card>
            ...
            <v-card class="mb-6 elevation-12">
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="primary" @click="resource_wizard = true"
      ...
      <v-dialog
        persistent
        max-width="450"
        v-model="resource_wizard"
        v-if="resource_wizard"
      >
        <v-card>
          <LinkResourceWizard
            @close="resource_wizard = false"
            @save="linkResource"
          />
        </v-card>
      </v-dialog>
...
export default {
  ...
  computed: {
    ...
    hasRights() {
      return (
        this.$root.user.role > 2 || this.$root.user.position == "Team Lead"
      );
    },
    filtered() {
      let data = [];
      ...
      switch (this.filter) {
        case "all":
          ...
          break;
        case "your":
          data = this.tasks.filter((i) =>
            i.assigned.includes(this.$root.user._id)
          );
          break;
        case "incomplete":
  ...
  methods: {
    ...
    save() {
      if (this.current) {
        this.$http
          .put(`/api/task/${this.current._id}`, this.current)
          .then((response) => {
    ...
    handleKeyUp(e) {
      if (e.code == "Escape" && this.$route.path != "/tasks" && !this.changes)
        this.$router.push("/tasks");
    },
  },
  created() {
