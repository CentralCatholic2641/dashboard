	...
	export default {
			...
			breadcrumbs() {
				return [
					...
					},
					{
						text: "",
						disabled: true,
						to: `/events/${this.event.year}${this.event.event_code.toUpperCase()}/teams/${this.team.team_number}`,
					},
				];
			},
		},
		methods: {
			refresh() {
				...
				this.$http.get(`/api/event/${this.$route.params.code.toLowerCase()}/teams/${this.$route.params.team}`).then((response) => {
