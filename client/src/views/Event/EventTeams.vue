				...
				<v-card class="mt-6 mb-4">
					<v-simple-table dense style="white-space: nowrap">
						<template v-slot:default>
							...
							<tbody>
								<tr v-ripple style="cursor: pointer" @click="$router.push(`/events/${event.year}${event.event_code.toUpperCase()}/teams/${team.team_number}`)" v-for="team in event.teams" :key="team.team_number">
	...
	export default {
			...
			breadcrumbs() {
				return [
					...
					},
					{
						text: "",
						disabled: true,
						to: `/events/${this.event.year}${this.event.event_code.toUpperCase()}/teams`,
					},
				];
			},
		},
		methods: {
			refresh() {
				...
				this.$http.get(`/api/event/${this.$route.params.code.toLowerCase()}/teams`).then((response) => {
