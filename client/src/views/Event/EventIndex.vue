	...
	<div v-if="event">
		<v-toolbar dense color="grey darken-4" style="z-index: 1">
			<v-toolbar-title class="d-flex">
				...
				<h6 class="text-h6 font-weight-black" v-if="$vuetify.breakpoint.mdAndUp">{{ event.year }} {{ event.name }}</h6>
		...
		<div :style="`height: calc(100vh - ${$vuetify.breakpoint.mdAndUp ? '112px' : '104px'}); overflow: auto`">
			<v-container style="max-width: 1500px" class="px-8 pt-0 mt-4" v-if="isActive">
			...
			<v-container class="px-8 pt-0 mt-4" v-else>
				<v-row class="mt-4">
					<v-col cols="12" md="6">
						<p class="font-italic grey--text mt-8 mb-8 pa-0 text-center">This event has not started yet</p>
					</v-col>
	...
	export default {
		...
		methods: {
			refresh() {
				...
				this.$http.get(`/api/event/${this.$route.params.code.toLowerCase()}`).then((response) => {
