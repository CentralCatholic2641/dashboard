    ...
    <v-toolbar dense color="grey darken-4" style="z-index: 1">
      <v-toolbar-title class="d-flex">
        ...
        <h6 class="text-h6 font-weight-black">
          <span v-if="match.comp_level != 'qm' && match.comp_level != 'f'"
            >{{ comp_levelMap[match.comp_level] }} {{ match.set_number }} Match
            {{ match.match_number }}</span
          >
          <span v-else
...
export default {
    ...
    breadcrumbs() {
      return [
        ...
        },
        {
          ...
          to: `/events/${
            this.event.year
          }${this.event.event_code.toUpperCase()}/schedule`,
        },
        {
          text: "",
          disabled: true,
          to: `/events/${
            this.event.year
          }${this.event.event_code.toUpperCase()}/schedule/${this.match.key
            .toUpperCase()
            .substring(this.match.key.indexOf("_") + 1)}`,
        },
      ];
    },
    start() {
      if (this.match)
        if (this.match.actual_time)
          return dayjs(this.match.actual_time * 1000).format(
            "dddd, MMMM D, YYYY [at] HH:mm"
          );
        else
          return dayjs(this.match.time * 1000).format(
            "dddd, MMMM D, YYYY [at] HH:mm"
          );
    },
  },
  methods: {
    refresh() {
      ...
      this.$http
        .get(
          `/api/event/${this.$route.params.code.toLowerCase()}/schedule/${this.$route.params.match.toLowerCase()}`
        )
        .then((response) => {
