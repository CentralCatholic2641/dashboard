<template>
	<div v-if="event && schedule && users">
		...
		<div :style="`height: calc(100vh - ${$vuetify.breakpoint.mdAndUp ? '112px' : '104px'}); overflow: auto`">
			<v-container style="max-width: 1185px" class="px-8 pt-0 mt-4" v-if="event.schedule.length != 0">
				<MatchScoutingController @refresh="refresh()" :event="event" :schedule="schedule" :users="users" v-if="$root.user.role > 1" />

				<MatchScoutingMember @refresh="refresh()" :event="event" :schedule="schedule" v-else />

				<PitScoutingController @refresh="refresh()" :event="event" :pitscouting="pitscouting" :users="users" v-if="$root.user.role > 1" />

				<PitScoutingMember @refresh="refresh()" :event="event" :pitscouting="pitscouting" v-else />

				<StatsByScouter @refresh="refresh()" :schedule="schedule" :pitscouting="pitscouting" :users="users" v-if="$root.user.role > 1" class="mb-3" />
			</v-container>

			<v-container style="max-width: 1185px" class="px-8 pt-0 mt-6" v-else>
				<v-card>
					<v-card-text>
						<p class="grey--text font-italic text-center mt-4">Scouting is not available for this competition</p>
					</v-card-text>
				</v-card>
			</v-container>
		</div>
			...
			breadcrumbs() {
				return [
					...
					},
					{
						text: "",
						disabled: true,
						to: `/events/${this.event.year}${this.event.event_code.toUpperCase()}/scouting`,
					},
				];
			},
		},
