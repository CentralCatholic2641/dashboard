  ...
  <div>
    ...
    >
      <v-container style="max-width: 1185px" class="px-8 pt-0 mt-4">
            ...
            <template v-slot:default>
              ...
              <tbody v-if="$root.user.role > 2">
                ...
                >
                  ...
                  <td>{{ payment.type }}</td>
                  <td>${{ payment.value }}</td>
                  <td :class="getPercentColor(getPercentIn(payment) * 100)">
                    {{ (getPercentIn(payment) * 100).toFixed(1) }}%
                  </td>
                </tr>
              </tbody>
              ...
              <tbody v-else>
                <tr
                  v-for="payment in payments.filter(
                    (i) => i.members[$root.user._id].included
                  )"
                  :key="payment._id"
                >
                  <td class="font-weight-black">
        ...
        <p v-if="payments.length > 0" class="pb-0 mt-4 mb-1 grey--text">
  ...
  methods: {
    ...
    getPercentIn(payment) {
      ...
      for (let i in payment.members) {
        ...
        if (
          payment.members[i].status == "Paid" ||
          payment.members[i].status == "Sponsored"
        )
          totalPaid++;
      }
